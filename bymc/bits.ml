(*
  Bit-vector structures encoded in SAT.
  Should make SAT encoding easier to us.

  Open this module with care as it introduces lots of shortcut symbols.
...
type bv_form = B1 | B0 | V of string | NV of string
...
let rec bv_form_s bv =
    match bv with
      ...
      | AND lst -> List.fold_left
        (fun s f ->
            (if s = "" then "(" else s ^ " & (") ^ (bv_form_s f) ^ ")") "" lst
      | OR lst -> List.fold_left
        (fun s f -> (if s = "" then "(" else s ^ " | (") ^ (bv_form_s f) ^ ")") "" lst
      | ANNOTATION(_, subform) -> bv_form_s subform
;;

let rec neg_bf_form bv =
    match bv with
        B1 -> B1
      | B0 -> B1
      | V x -> NV x
      ...
      | ANNOTATION(text, subform) -> ANNOTATION(text, neg_bf_form subform)
;;


let rec format_bv_form ff bv =
    match bv with
