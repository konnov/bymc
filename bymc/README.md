...
Follow the messages by the script. If you want to provide an invariant, then introduce one like ```tx_inv``` in ```bcast_symb```.
...
The script ```ocamlbrew``` bootstraps the whole ocaml infrastructure together with the package
