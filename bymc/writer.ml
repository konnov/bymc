...
let openb ff lvl = Format.pp_open_hovbox ff lvl;;
let closeb ff = Format.pp_close_box ff (); Format.pp_print_newline ff ();;
...
let rec write_stmt ff lvl lab_tab s =
    match s with
    | MSkip _ ->
        openb ff lvl;
        ...
        closeb ff
    | MExpr (_, e) ->
        openb ff lvl; fprint_expr ff e; Format.pp_print_string ff ";"; closeb ff
    | MDecl (_, v, e) ->
        openb ff lvl;
        let print_flags =
            let pf f =
    ...
    | MAtomic (_, seq) ->
        openb ff lvl; Format.pp_print_string ff "atomic {"; closeb ff;
        List.iter (write_stmt ff (lvl + 2) lab_tab) seq;
        openb ff lvl; Format.pp_print_string ff "}"; closeb ff
    | MD_step (_, seq) ->
        openb ff lvl; Format.pp_print_string ff "d_step {"; closeb ff;
        List.iter (write_stmt ff (lvl + 2) lab_tab) seq;
        openb ff lvl; Format.pp_print_string ff "}"; closeb ff
    | MGoto (_, l) ->
        openb ff lvl;
    ...
    | MIf (_, opts) ->
        ...
        List.iter
            (function
                | MOptGuarded seq ->
                    openb ff lvl; Format.fprintf ff "@ @ ::@ @]";
                    write_stmt ff 0 lab_tab (List.hd seq);
                    List.iter
                        (write_stmt ff (lvl + 4) lab_tab)
                        (List.tl seq);

                | MOptElse seq ->
                    let semi = if (List.length seq) = 0 then "" else "->" in
                    openb ff lvl; Format.fprintf ff "@ @ :: else@ %s" semi;
                    closeb ff;
                    List.iter (write_stmt ff (lvl + 4) lab_tab) seq;
            ) opts;
        openb ff lvl; Format.pp_print_string ff "fi;"; closeb ff;
    | MAssert (_, e) ->
        openb ff lvl;
...
let write_proc ff lvl p =
    ...
    List.iter (write_stmt ff (lvl + 2) labels) p#get_stmts;
    (* end the body *)
...
let write_unit cout lvl u =
    ...
    match u with
    | Stmt s ->
        write_stmt ff lvl (Hashtbl.create 0) s
    | Proc p ->
        write_proc ff lvl p
    | Ltl (name, exp) ->
        openb ff lvl;
